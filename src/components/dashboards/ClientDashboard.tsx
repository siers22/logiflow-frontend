      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">Управление и отслеживание заявок на перевозку</p>
          </div>
          <Dialog open={isNewOrderOpen} onOpenChange={setIsNewOrderOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>Заполните форму для создания заявки</DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="pickup">Адрес отправления</Label>
                    <Input id="pickup" placeholder="Москва, ул. Ленинская, 15" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="delivery">Адрес доставки</Label>
                    <Input id="delivery" placeholder="Санкт-Петербург, пр. Невский, 28" />
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="notes" placeholder="Дополнительная информация о грузе" rows={3} />
                </div>
                <div className="flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => setIsNewOrderOpen(false)}>Отмена</Button>
                  <Button onClick={() => setIsNewOrderOpen(false)}>Создать заявку</Button>
                </div>
              </div>
            </DialogContent>
          </Dialog>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Отслеживайте статус ваших перевозок</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {orders.map((order) => {
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {order.status === 'in_progress' && (
                            <span className="text-xs text-blue-600 dark:text-blue-400 group-hover:underline">Отследить →</span>
                          )}
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{order.cargoType} • {order.weight} кг</p>
                      </div>
