...
export function OrderDetailsPage({ orderId }: OrderDetailsPageProps) {
  ...
  const deliveryCoords =
    cityCoordinates[deliveryCity] ?? cityCoordinates["Санкт-Петербург"];
  const progress = getDeliveryProgress(order.status);
  const currentProgress =
    order.status === "in_progress" ? cargoProgress : progress;

  const currentLat =
    pickupCoords.lat +
    (deliveryCoords.lat - pickupCoords.lat) * (currentProgress / 100);
  const currentLng =
    pickupCoords.lng +
    (deliveryCoords.lng - pickupCoords.lng) * (currentProgress / 100);
  ...
  const estimatedHours = Math.ceil(
    (order.distance / 80) * (1 - currentProgress / 100),
  );
  const eta = new Date();
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2" variant="glass">
            <CardHeader>
              ...
              <p className="text-sm text-gray-500">
                {pickupCity} → {deliveryCity} • {order.distance} км
              </p>
            </CardHeader>
            <CardContent>
              <div
                ...
                >
                  ...
                  {order.status === "in_progress" && (
                    <g transform={`translate(${cp.x},${cp.y})`}>
                      ...
                      <path
                        d="M -4 -2 L 4 -2 L 4 2 L 2 2 L 2 6 L -2 6 L -2 2 L -4 2 Z"
                        fill="white"
                      />
                    </g>
                  )}
          ...
          <div className="space-y-6">
            ...
            {order.status === "in_progress" && (
              <Card variant="glass">
                <CardHeader>
                  <CardTitle>Расчетное время прибытия</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                        {eta.toLocaleTimeString("ru-RU", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </div>
                      <div className="text-xs text-gray-500">
                        {eta.toLocaleDateString("ru-RU")}
                      </div>
                      <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                        Примерно через {estimatedHours}{" "}
                        {estimatedHours === 1
                          ? "час"
                          : estimatedHours < 5
                            ? "часа"
